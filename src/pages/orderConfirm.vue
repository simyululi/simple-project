...
export default{
  ...
  methods:{
    ...
    submitAddress(){
      ...
      _submitAddress(url, method,params).then(()=>{
        _this.closeModal();
    ...
    getCartList(){
      const _this = this
      _getCartList().then((res)=>{
        let list = res.cartProductVoList;//获取购物车中所有商品数据
        _this.cartTotalPrice = res.cartTotalPrice;//商品总金额
        _this.cartList = list.filter(item=>item.productSelected);
